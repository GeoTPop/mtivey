...
% INV2D Calculate magnetization from magnetic field and bathymetry
% for a constant thickness source layer. Use the Parker & Huestis [1974] 
% Fourier inversion approach.
% 
...
if nargin < 1
 help inv2d
 % test a simple variable layer model with constant magnetization
 figure(1)
 m2d=zeros(256,1)+10;m2d(120:140)=20;  
 h=ones(size(m2d)).*(-4);
 thk=0.5;
 dx=0.5;rlat=26;rlon=-45;yr=1990;slin=20;zobs=0;
 f2d=syn2d(m2d,h,rlat,rlon,yr,zobs,thk,slin,dx);
 fprintf('\n\nForward Model generated now Hit Enter to continue ');pause
 % now do inversion
 figure(2)
 wl=0,ws=0;
 m2da=inv2d(f2d,h,wl,ws,rlat,rlon,yr,zobs,thk,slin,dx); 
 return
end
%
...
if flag == 1, 
  disp(' I would be quitting now, error diverging ');
else
